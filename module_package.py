...
def status_log(r):
    ...
    with open(url_log_file, 'r') as read_file:
        read_file.write(f'{r.url}, {r.status_code}\n')
...
def get_soup(url, headers=None):
    r = requests.Session().get(url, headers=headers, timeout=30)
...
def post_soup(url, headers=None, payload=None):
    ...
    if r.status_code == 200:
        ...
        while count != 10:
            print('while', count)
            r = requests.Session().post(url, headers=headers, json=payload)
            print('status_code: ', r.status_code)
            if r.status_code == 200:
                soup = BeautifulSoup(r.text, 'html.parser')
                return soup
...
def post_json_response(url, headers=None, payload=None):
    ...
    if r.status_code == 200:
        ...
        while count != 10:
            print('while', count)
            r = ses.post(url, headers=headers, data=payload)
            print('status_code: ', r.status_code)
            if r.status_code == 200:
                return r.json()
            else:
...
def get_dictionary(product_names=None, product_urls=None, product_prices=None):
    dictionary_1 = {
